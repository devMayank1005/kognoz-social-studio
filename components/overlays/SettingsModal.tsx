...
import { X, User, Building, Download, Command, type LucideIcon } from "lucide-react";
    ...
    >
      ...
      >
        <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <h3 className="font-semibold text-sm text-slate-900">Workspace settings</h3>
          <button
            onClick={onClose}
            aria-label="Close"
          ...
          <div className="flex-1 overflow-y-auto p-5 text-xs text-slate-600 leading-relaxed min-w-0">
            {tab === "account" && (
              <Pane title="Account & access">
                <Field label="Signed in as" value={session?.user?.name || "—"} />
                <Field label="Email" value={session?.user?.email || "—"} mono />
                <Field label="Role" value={session?.user?.isAdmin ? "Administrator" : "Editor"} />
                <Note>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-3 rounded-xl border text-left transition-all ${
                          on ? "border-[#43AFCD] bg-[#43AFCD]/5" : "border-slate-200 hover:border-slate-300"
                        }`}
                      >
                        ...
                        {on && <span className="text-[10px] font-mono text-[#0A6E8F] shrink-0">active</span>}
                      </button>
                    );
                  })}
                </div>
            ...
            {tab === "export" && (
              <Pane title="Export defaults">
                <Field label="Slide size" value="1080 × 1350 for 4:5 formats" mono />
                <Field label="Fonts" value="Embedded in every file" />
                <Field label="PNG carousel" value="One file per slide, saved individually" />
                <Note>
